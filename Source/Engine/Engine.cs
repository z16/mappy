...
namespace MapEngine
{
    ...
    public partial class Engine : Component
    {
        ...
        private Color m_HiddenColor = Color.Gray;
        private Color m_HiddenTextColor = Color.Gray;
        private Font m_HiddenTextFont = SystemFonts.DefaultFont;
        private float m_HiddenSize = 4.0f;
        ...
        private Brush bTextHidden = new SolidBrush(Color.Gray);
        ...
        [Category("Map")]
        [Description("Gets or sets the color of spawns that are not rendered by the game client.")]
        ...
        [Category("Map")]
        [Description("Gets or sets the font used to display hidden spawn information.")]
        [DisplayName("Hidden Font")]
        public Font HiddenInfoFont
        {
            get { return m_HiddenTextFont; }
            set
            {
                m_HiddenTextFont = value;
                if (Updated != null)
                    Updated();
            }
        }
        ...
        [Category("Map")]
        [Description("Gets or sets the color used to display hidden spawn information.")]
        [DisplayName("Hidden Info Color")]
        public Color HiddenInfoColor
        {
            get { return m_HiddenTextColor; }
            set
            {
                m_HiddenTextColor = value;
                bTextHidden = new SolidBrush(value);
            }
        }

        [Category("Map")]
        [Description("Gets or Sets the base rendering size of a Hidden spawn.")]
        [DisplayName("Hidden Size")]
        [DefaultValue(4.0f)]
        public float HiddenSize
        {
            get { return m_HiddenSize; }
            set
            {
                m_HiddenSize = value;
                if (Updated != null)
        ...
        private void DrawSpawns(Graphics g)
        {
            ...
            foreach (KeyValuePair<uint, GameSpawn> pair in m_game.Spawns)
            {
                ...
                if (spawn != m_game.Player && spawn != m_game.Target && spawn != m_game.Selected)
                {
        ...
        private void DrawSpawn(Graphics g, GameSpawn spawn)
        {
            ...
            bool STH = spawn == m_game.Selected || spawn == m_game.Target || spawn == m_game.Highlighted;
            bool STHAH = STH || spawn.Alert || spawn.Hunt;
            bool STHP = STH || spawn == m_game.Player;
                ...
                else
                {
                    ...
                    switch (spawn.Type)
                    {
                        ...
                        case SpawnType.MOB:
                            ...
                            break;
                        case SpawnType.Hidden:
                            g.FillEllipse(bHidden, x - radius, y - radius, size, size);
                            break;
                    }
                }

                //draw heading line if required
                if (m_drawHeadings || STHAH)
                {
                ...
                if (showInfo)
                {
                    ...
                    switch (spawn.Type)
                    {
                        ...
                        case SpawnType.MOB:
                            ...
                            break;
                        case SpawnType.Hidden:
                            fInfoText = m_HiddenTextFont;
                            bInfoText = bTextHidden;
                            break;
                        default:
